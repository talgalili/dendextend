...
##  [7] head.dendrogram*       labels.dendrogram*    
##  [9] labels<-.dendrogram    merge.dendrogram*     
## [11] nleaves.dendrogram*    nnodes.dendrogram*    
## [13] plot.dendrogram*       print.dendrogram*     
## [15] reorder.dendrogram*    rev.dendrogram*       
## [17] rotate.dendrogram*     sort.dendrogram*      
## [19] str.dendrogram*        trim.dendrogram*      
## [21] unbranch.dendrogram*  
## 
...
##         nleaves(dend)  24.08  26.04  27.16  30.8 132.7
##  length(labels(dend)) 390.83 394.19 400.35 418.0 535.9
##  neval
##    100
##    100
\end{verbatim}
...
##         nleaves(hc) 17.92 18.48  19.04 19.60  31.92   100
##  length(labels(hc)) 30.80 31.36  31.92 31.92 131.59   100
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\end{verbatim}
\begin{alltt}
\hlcomment{# while we have 3 leaves, in practice we have only 2}
\hlcomment{# terminal nodes (this is a feature, not a bug.)}
...
\hlfunctioncall{microbenchmark}(get_leaves_attr_4members = \hlfunctioncall{get_leaves_attr}(dend, 
    \hlstring{"members"}), get_nodes_attr_4members = \hlfunctioncall{get_nodes_attr}(dend, 
    \hlstring{"members"}, include_branches = FALSE, na.rm = TRUE))
\end{alltt}
\begin{verbatim}
## Unit: microseconds
##                      expr   min    lq median    uq  max
##  get_leaves_attr_4members 288.9 296.5  309.1 327.6 3543
##   get_nodes_attr_4members 707.8 720.4  745.0 789.2 3127
##  neval
##    100
##    100
\end{verbatim}
...
unbranched_dend <- \hlfunctioncall{unbranch}(dend, branch_becoming_root = 1)
unbranched_dend_2 <- \hlfunctioncall{unbranch}(unbranched_dend, branch_becoming_root = 3)

\hlfunctioncall{par}(mfrow = \hlfunctioncall{c}(1, 3))
\hlfunctioncall{plot}(dend)
\hlfunctioncall{plot}(unbranched_dend)
\hlfunctioncall{plot}(unbranched_dend_2)
\end{alltt}
\end{kframe}
...
\hlcomment{# removing color (and the nodePar completely - if it has no}
\hlcomment{# other attributed but lab.col)}
\hlfunctioncall{labels_colors}(dend) <- NULL
\end{alltt}


{\ttfamily\noindent\color{warningcolor}{\#\# Warning: Length of color vector was shorter than the number of leaves - vector color recycled}}
...
\hlfunctioncall{plot}(trimmed_dend_1, main = \hlstring{"Trimmed tree - single method"}, ylim = \hlfunctioncall{c}(0, 
    110))
\hlfunctioncall{plot}(trimmed_dend_2, main = \hlstring{"Trimmed tree - complete method"}, 
    ylim = \hlfunctioncall{c}(0, 110))
\hlfunctioncall{plot}(dends_12[[1]], main = \hlstring{"Intersected tree - single method"}, 
    ylim = \hlfunctioncall{c}(0, 110))
\hlfunctioncall{plot}(dends_12[[2]], main = \hlstring{"Intersected tree - complete method"}, 
    ylim = \hlfunctioncall{c}(0, 110))
\end{alltt}
\end{kframe}
...
\hlcomment{# let's color the labels to make the followup of the}
\hlcomment{# rotation easier}
...
hc <- \hlfunctioncall{hclust}(\hlfunctioncall{dist}(USArrests[\hlfunctioncall{c}(1, 6, 13, 20, 23), ]), \hlstring{"ave"})
dend <- \hlfunctioncall{as.dendrogram}(hc)
unbranch_dend <- \hlfunctioncall{unbranch}(dend, 2)
...
\hlcomment{# cutree now works for unbranched trees!}
\hlcomment{# as.hclust(unbranch_dend) # ERROR - can not do this...}
\hlfunctioncall{cutree}(unbranch_dend, k = 2)  \hlcomment{# all NA's}
\end{alltt}
...
\end{verbatim}
\begin{alltt}
\hlfunctioncall{cutree}(unbranch_dend, k = 1:4)
\end{alltt}
...
\end{verbatim}
\begin{alltt}
\hlfunctioncall{cutree}(unbranch_dend, h = \hlfunctioncall{c}(20, 25.5, 50, 170))
\end{alltt}
\begin{verbatim}
##           20 25.5 50 170
## Alabama    1    1  2   2
...
\hlfunctioncall{require}(microbenchmark)
\hlcomment{## this shows how as.hclust is expensive - but still worth}
\hlcomment{## it if possible}
\hlfunctioncall{microbenchmark}(\hlfunctioncall{cutree}(hc, k = 2:4), \hlfunctioncall{cutree}(\hlfunctioncall{as.hclust}(dend), k = 2:4), 
    \hlfunctioncall{cutree}(dend, k = 2:4), \hlfunctioncall{cutree}(dend, k = 2:4, try_cutree_hclust = FALSE))
\end{alltt}
\begin{verbatim}
## Unit: microseconds
##                                              expr    min
##                               cutree(hc, k = 2:4)  134.9
##                  cutree(as.hclust(dend), k = 2:4) 1738.0
##                             cutree(dend, k = 2:4) 1865.1
##  cutree(dend, k = 2:4, try_cutree_hclust = FALSE) 8250.6
##      lq median     uq     max neval
##   144.5  147.8  199.3   357.2   100
##  1788.1 1851.1 1892.3  4769.5   100
##  1935.4 1991.4 2050.2  4888.2   100
##  8485.5 8676.1 8947.7 99834.4   100
\end{verbatim}
\begin{alltt}
\hlcomment{# the dendrogram is MUCH slower...}


\hlcomment{# and trying to 'hclust' is not expensive (which is}
\hlcomment{# nice...)}
\hlfunctioncall{microbenchmark}(cutree_unbranch_dend = \hlfunctioncall{cutree}(unbranch_dend, k = 2:4, 
    warn = FALSE), cutree_unbranch_dend_not_trying_to_hclust = \hlfunctioncall{cutree}(unbranch_dend, 
    k = 2:4, try_cutree_hclust = FALSE, warn = FALSE))
\end{alltt}
\begin{verbatim}
## Unit: milliseconds
##                                       expr   min    lq
##                       cutree_unbranch_dend 6.786 6.924
##  cutree_unbranch_dend_not_trying_to_hclust 6.359 6.504
##  median    uq   max neval
##   7.064 7.197 33.35   100
##   6.614 6.916 13.67   100
\end{verbatim}
\begin{alltt}
...
d_iris <- \hlfunctioncall{dist}(iris[, -5])
hc_iris <- \hlfunctioncall{hclust}(d_iris)
\hlfunctioncall{labels}(hc_iris)
\end{alltt}
\begin{verbatim}
## character(0)
\end{verbatim}
\begin{alltt}
dend_iris <- \hlfunctioncall{as.dendrogram}(hc_iris)
\hlfunctioncall{is.integer}(\hlfunctioncall{labels}(dend_iris))  \hlcomment{# this could cause problems...}
\end{alltt}
\begin{verbatim}
## [1] TRUE
\end{verbatim}
...
iris_species <- \hlfunctioncall{rev}(\hlfunctioncall{levels}(iris[, 5]))
dend_iris <- \hlfunctioncall{color_branches}(dend_iris, k = 3, groupLabels = iris_species)
\hlfunctioncall{is.character}(\hlfunctioncall{labels}(dend_iris))
\end{alltt}
\begin{verbatim}
## [1] TRUE
\end{verbatim}
...
\hlcomment{# have the labels match the real classification of the}
\hlcomment{# flowers:}
\hlfunctioncall{labels_colors}(dend_iris) <- \hlfunctioncall{rainbow_hcl}(3)[\hlfunctioncall{sort_levels_values}(\hlfunctioncall{as.numeric}(iris[, 
    5])[\hlfunctioncall{order.dendrogram}(dend_iris)])]

\hlcomment{# We'll add the flower type}
\hlfunctioncall{labels}(dend_iris) <- \hlfunctioncall{paste}(\hlfunctioncall{as.character}(iris[, 5])[\hlfunctioncall{order.dendrogram}(dend_iris)], 
    \hlstring{"("}, \hlfunctioncall{labels}(dend_iris), \hlstring{")"}, sep = \hlstring{""})

dend_iris <- \hlfunctioncall{hang.dendrogram}(dend_iris, hang_height = 0.1)

\hlcomment{# reduce the size of the labels:}
dend_iris <- \hlfunctioncall{assign_values_to_leaves_nodePar}(dend_iris, 0.5, 
    \hlstring{"lab.cex"})
\end{alltt}
...
\hlfunctioncall{par}(mar = \hlfunctioncall{c}(3, 3, 3, 7))
\hlfunctioncall{plot}(dend_iris, main = \hlstring{"Clustered Iris dataset\textbackslash{}\hlfunctioncall{n}(the labels give the true flower species)"}, 
    horiz = TRUE, nodePar = \hlfunctioncall{list}(cex = 0.007))
\hlfunctioncall{legend}(\hlstring{"topleft"}, legend = iris_species, fill = \hlfunctioncall{rainbow_hcl}(3))
\end{alltt}
\end{kframe}
...
species_col <- \hlfunctioncall{rev}(\hlfunctioncall{rainbow_hcl}(3))[\hlfunctioncall{as.numeric}(iris[, 5])]
\hlcomment{# species_col[c(107,71)] <- 'red' # highlight the two}
\hlcomment{# extreme cases we mentioned}
species_pch <- \hlfunctioncall{rep}(1, \hlfunctioncall{length}(species_col))
species_pch[\hlfunctioncall{c}(107, 71)] <- 19  \hlcomment{# highlight the two extreme cases we mentioned}

\hlfunctioncall{pairs}(iris[, -5], col = species_col, pch = species_pch)
\end{alltt}
\end{kframe}
