...
##  [3] as.phylo.dendrogram*        
##  [4] cophenetic.dendrogram*      
##  [5] cor_bakers_gamma.dendrogram*
##  [6] cut.dendrogram*             
##  [7] cutree.dendrogram*          
##  [8] entanglement.dendrogram*    
##  [9] head.dendrogram*            
## [10] labels.dendrogram           
## [11] labels<-.dendrogram*        
## [12] merge.dendrogram*           
## [13] nleaves.dendrogram*         
## [14] nnodes.dendrogram*          
## [15] plot.dendrogram*            
## [16] print.dendrogram*           
## [17] prune.dendrogram*           
## [18] reorder.dendrogram*         
## [19] rev.dendrogram*             
## [20] rotate.dendrogram*          
## [21] shuffle.dendrogram*         
## [22] sort.dendrogram*            
## [23] str.dendrogram*             
## [24] tanglegram.dendrogram*      
## [25] unbranch.dendrogram*        
## 
...
\end{verbatim}
\begin{alltt}
\hlstd{hc} \hlkwb{<-} \hlkwd{hclust}\hlstd{(}\hlkwd{dist}\hlstd{(US_data),} \hlstr{"ave"}\hlstd{)}  \hlcom{# create an Hierarchical clustering object}
...
\hlstd{dend_from_dendrapply} \hlkwb{<-} \hlkwd{dendrapply}\hlstd{(dend, itself)}
...
##         nleaves(dend) 26.32 26.88  28.00 30.80 113.1   100
##  length(labels(dend)) 71.11 72.23  72.79 74.19 194.3   100
\end{verbatim}
\begin{alltt}
...
## Unit: microseconds
##                expr   min    lq median    uq    max neval
##         nleaves(hc) 21.28 22.40  22.96 23.52  87.35   100
##  length(labels(hc)) 34.72 35.84  36.40 36.96 267.09   100
\end{verbatim}
\end{kframe}
\end{knitrout}
...
##  get_leaves_attr_4members 309.6 318.0  334.3 354.2 6709
##   get_nodes_attr_4members 759.8 791.7  839.3 886.1 9320
##  neval
##    100
##    100
\end{verbatim}
...
\hlstd{flatten_dend_2} \hlkwb{<-} \hlkwd{flatten.dendrogram}\hlstd{(dend,} \hlkwc{FUN} \hlstd{= min)}
...
\begin{alltt}
\hlkwd{require}\hlstd{(colorspace)}
\hlkwd{labels_colors}\hlstd{(hang_dend_1)} \hlkwb{<-} \hlkwd{rainbow_hcl}\hlstd{(}\hlkwd{nleaves}\hlstd{(hang_dend_1))}
...
\hlstd{pruned_dend} \hlkwb{<-} \hlkwd{prune}\hlstd{(dend,} \hlkwd{c}\hlstd{(}\hlstr{"Alaska"}\hlstd{,} \hlstr{"California"}\hlstd{))}
...
\hlstd{dend_2} \hlkwb{<-} \hlkwd{as.dendrogram}\hlstd{(hc_2)}
...
\hlstd{pruned_dend_2} \hlkwb{<-} \hlkwd{prune}\hlstd{(dend_2,} \hlkwd{c}\hlstd{(}\hlstr{"California"}\hlstd{))}

\hlstd{dends_12} \hlkwb{<-} \hlkwd{intersect_trees}\hlstd{(pruned_dend_1, pruned_dend_2)}
...
## Unit: milliseconds
##                                              expr   min
##                               cutree(hc, k = 2:4) 1.541
##                  cutree(as.hclust(dend), k = 2:4) 3.510
##                             cutree(dend, k = 2:4) 3.581
##  cutree(dend, k = 2:4, try_cutree_hclust = FALSE) 3.502
##     lq median    uq    max neval
##  1.633  1.684 1.853 133.69   100
##  3.745  3.974 4.268  15.13   100
##  3.809  4.067 4.350  10.28   100
##  3.776  4.075 4.565  11.28   100
\end{verbatim}
\begin{alltt}
...
## Unit: milliseconds
##                                       expr   min    lq
##                       cutree_unbranch_dend 2.166 2.305
##  cutree_unbranch_dend_not_trying_to_hclust 1.622 1.786
##  median    uq   max neval
##   2.397 2.813 8.286   100
##   1.876 2.213 5.851   100
\end{verbatim}
\begin{alltt}
...
\hlkwd{labels_colors}\hlstd{(dend_iris)} \hlkwb{<-} \hlkwd{rainbow_hcl}\hlstd{(}\hlnum{3}\hlstd{)[}\hlkwd{sort_levels_values}\hlstd{(}\hlkwd{as.numeric}\hlstd{(iris[,}
    \hlnum{5}\hlstd{])[}\hlkwd{order.dendrogram}\hlstd{(dend_iris)])]}
...
This simple visualization easily demonstrates how the seperation of the Hierarchical clustering is very good with the "setosa" sepcies, but misses in labeling many "versicolor" species as "virginica".
...
\hlcom{# nicely}
\hlkwd{labels}\hlstd{(dend2)} \hlkwb{<-} \hlkwd{as.character}\hlstd{(}\hlkwd{labels}\hlstd{(dend2))}
...
\hlstd{s_dend2} \hlkwb{<-} \hlkwd{sort}\hlstd{(dend2)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{best_dend_heights_per_k} \hlkwb{<-} \hlkwd{heights_per_k.dendrogram}\hlstd{(dend2)}
...
## Unit: milliseconds
##         expr   min    lq median    uq   max neval
##  with_labels 35.05 39.34  41.25 42.94 49.30    10
##  with_values 32.73 36.68  39.69 42.01 46.64    10
\end{verbatim}
\begin{alltt}
...
{\centering \includegraphics[width=\maxwidth]{figure/cor_bakers_gamma_simulation_CI_42} 
...
## Unit: milliseconds
##              expr   min    lq median    uq   max neval
##  cor_bakers_gamma 37.12 39.16  41.04 45.66 47.37    10
##    cor_cophenetic 13.74 14.77  15.15 15.91 22.36    10
\end{verbatim}
\begin{alltt}
...
## Unit: milliseconds
##              expr   min    lq median    uq   max neval
##  cor_bakers_gamma 27.73 29.21  30.78 32.51 42.05    10
##    cor_cophenetic 13.85 14.37  16.09 19.57 23.83    10
\end{verbatim}
\end{kframe}
\end{knitrout}
...
##      Bk(hc1, hc2, k = 3)   5.784   5.854   6.163   6.44
##  Bk(dend1, dend2, k = 3) 189.247 193.771 199.897 214.49
##      max neval
##    6.508    10
##  258.647    10
\end{verbatim}
\begin{alltt}
\hlcom{# however, because the hc2 tree is actually with several}
\hlcom{# branches/leaves of the same height - actually the}
\hlcom{# dendrogram object gives a more correct output!}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlkwd{labels}\hlstd{(dend)} \hlkwb{<-} \hlkwd{as.character}\hlstd{(}\hlkwd{labels}\hlstd{(dend))}
...
## Unit: microseconds
##                                     expr     min      lq
##          stats:::labels.dendrogram(dend) 22113.4 22871.6
##  dendextendRcpp::labels.dendrogram(dend)   567.2   600.2
##   median      uq    max neval
##  23467.4 25423.2 147528   100
##    731.6   764.3   1143   100
\end{verbatim}
\begin{alltt}
\hlcom{# Rcpp is ~34 times faster}
\end{alltt}
\end{kframe}
\end{knitrout}
...
## Unit: microseconds
##                                        expr   min    lq
##              old_get_branches_heights(dend) 479.3 491.1
##  dendextendRcpp::get_branches_heights(dend) 521.3 549.6
##  median    uq  max neval
##   516.8 566.7  990   100
##   581.8 666.6 1316   100
\end{verbatim}
\begin{alltt}
\hlcom{# Rcpp is ~90 times faster!}
\end{alltt}
\end{kframe}
\end{knitrout}
...
## Unit: milliseconds
##                                            expr   min    lq
##              old_heights_per_k.dendrogram(dend) 25.66 26.10
##  dendextendRcpp::heights_per_k.dendrogram(dend) 25.70 26.22
##  median    uq   max neval
##   26.56 27.82 57.47   100
##   26.64 27.86 36.43   100
\end{verbatim}
\begin{alltt}
\hlcom{# Rcpp is ~135 times faster!}
\end{alltt}
\end{kframe}
\end{knitrout}
...
## Unit: milliseconds
##                                           expr    min
##              old_cut_lower_fun(dend_big, 0.01) 44.809
##  dendextendRcpp::cut_lower_fun(dend_big, 0.01)  4.094
##      lq median     uq    max neval
##  51.094 54.616 59.631 107.52   100
##   4.334  4.586  4.847  73.35   100
\end{verbatim}
\begin{alltt}
\hlcom{# Rcpp is ~4-14 times faster. depending on the size of the}
\hlcom{# tree, and the depth of the height}
\end{alltt}
\end{kframe}
\end{knitrout}
...
## [1] dendextendRcpp_0.3.0 Rcpp_0.10.6         
## [3] dendextend_0.12.0    ape_3.0-11          
## [5] knitr_1.5            installr_0.13.0     
## 
## loaded via a namespace (and not attached):
## [1] digest_0.6.4    evaluate_0.5.1  formatR_0.10   
## [4] grid_3.0.2      lattice_0.20-24 nlme_3.1-113   
