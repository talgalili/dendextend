...
Coloring labels can sometimes be useful, it is done through the \code{labels_colors} function (which also has assignment). Notice the assignment recycling, as well as the difference in the appearance of a dot when labels' color is black, compared to when it is NULL:
...
plot(pruned_dend, main = "Tree without Alaska and California")
...
dend2_2 <- untangle_step_rotate_1side(dend2, dend1,
               k_seq=2:3, best_dend_heights_per_k=best_dend_heights_per_k) 
...
Sometimes, using just one-side greedy algorithm is not enough. Also, it is probably best to combine the random search with the two sided greedy step-wise search:
...
require(dendextendRcpp) # already loaded by default with dendextend
require(microbenchmark)
microbenchmark(
...
require(microbenchmark)
microbenchmark(old_cut_lower_fun(dend_big,.01),
               dendextendRcpp::cut_lower_fun(dend_big,.01),
               times = 100)
# Rcpp is ~4-14 times faster. depending on the size of the tree, 
# and the depth of the height
