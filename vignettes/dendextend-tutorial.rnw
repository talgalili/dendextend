...
\begin{enumerate}
   ...
   \item Other tree objects (such as \code{hclust}, and objects from the \pkg{ape} package) include an \code{as.dendrogram] method for converting their objects into a dendrogram.
\end{enumerate}
   \item \code{dendrogram} objects are used in various packages as an intermediate step for other purposes (often plotting). They are used in packages such as: \citep{CRAN:latticeExtra} (see the \code{dendrogramGrob} function), \citep{CRAN:labeltodendro} (see the \code{colorplot} function), \citep{CRAN:bclust} (see the \code{bclust} function), \citep{CRAN:ggdendro} (see the \code{dendro_data} function), \citep{CRAN:Heatplus} (see the \code{annHeatmap2} function), \citep{CRAN:sparcl} (see the \code{ColorDendrogram} function).
...
This was changed in the beginning of 2013 thanks to Gregory Jefferis's \pkg{dendroextras} package \citep{CRAN:dendroextras}, which orgranized this through the \code{colour_clusters} function. In the \pkg{dendextend} package I will mostly import his code, with some modifications.
